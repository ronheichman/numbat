...
func TestReleasePrecisionPrivilegePolicy(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"clustered attached visudo validation is not a modification", cmd("visudo -qcf/etc/sudoers"), ""},
		{"visudo help is not a modification", cmd("visudo --help"), ""},
		{"visudo export is not a modification", cmd("visudo --export=/tmp/sudoers.json"), ""},
		{"short visudo export is not a modification", cmd("visudo -f /etc/sudoers -x /tmp/sudoers.json"), ""},
		{"visudo against a repository fixture stays quiet", cmd("visudo -f ./testdata/sudoers"), ""},
		{"short visudo file argument beginning with equals stays relative", cmd("visudo -f=/etc/sudoers"), ""},
		...
		{"visudo attached short file against active policy", cmd("visudo -f/etc/sudoers.d/agent"), "privilege.sudoers_tamper"},
		{"visudo clustered file option against active policy", cmd("visudo -qf /etc/sudoers.d/agent"), "privilege.sudoers_tamper"},
		{"visudo clustered attached file against active policy", cmd("visudo -qf/etc/sudoers.d/agent"), "privilege.sudoers_tamper"},
		{"visudo clustered edit options target default policy", cmd("visudo -qs"), "privilege.sudoers_tamper"},
		...
		{"sudoers copy target before trailing option", cmd("cp /tmp/policy /etc/sudoers.d/agent --force"), "privilege.sudoers_tamper"},
		{"sudoers copy target directory", cmd("cp -t /etc/sudoers.d /tmp/agent"), "privilege.sudoers_tamper"},
		{"sudoers copy inline target directory", cmd("cp --target-directory=/proc/self/root/etc/sudoers.d /tmp/agent"), "privilege.sudoers_tamper"},
		{"sudoers copy positional target directory", cmd("cp /tmp/agent /etc/sudoers.d"), "privilege.sudoers_tamper"},
		{"target directory option value is not a copy source", cmd("cp -t /etc/sudoers.d --suffix agent"), ""},
		{"sudoers copy source before trailing option stays quiet", cmd("cp /etc/sudoers /tmp/policy --force"), ""},
		{"sudoers copy source with target directory stays quiet", cmd("cp -t /tmp /etc/sudoers"), ""},
		{"later target-directory option overrides apparent sudoers destination", cmd("cp /tmp/policy /etc/sudoers --target-directory /tmp"), ""},
		{"later target-directory option overrides protected target directory", cmd("cp -t /etc/sudoers.d -t /tmp /tmp/agent"), ""},
		{"last protected target-directory option wins", cmd("cp -t /tmp -t /etc/sudoers.d /tmp/agent"), "privilege.sudoers_tamper"},
...
func TestReleasePrecisionPersistencePolicy(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"authorized keys copy target before trailing option", cmd(`cp /tmp/keys ~/.ssh/authorized_keys --force`), "persistence.ssh_authorized_keys_command"},
		{"authorized keys copy into positional directory", cmd(`cp /tmp/authorized_keys ~/.ssh`), "persistence.ssh_authorized_keys_command"},
		{"authorized keys copy into target directory", cmd(`cp -t ~/.ssh /tmp/authorized_keys`), "persistence.ssh_authorized_keys_command"},
		...
		{"last ssh target-directory option wins", cmd(`cp --target-directory=/tmp --target-directory=~/.ssh /tmp/authorized_keys`), "persistence.ssh_authorized_keys_command"},
		{"unrelated file copied into ssh directory stays quiet", cmd(`cp /tmp/notes ~/.ssh`), ""},
		{"backup suffix is not an authorized keys source", cmd(`cp -t ~/.ssh --backup --suffix authorized_keys /tmp/notes`), ""},
		{"authorized keys copy source before trailing option stays quiet", cmd(`cp ~/.ssh/authorized_keys /tmp/keys --force`), ""},
		{"authorized keys copy source with target directory stays quiet", cmd(`cp -t /tmp ~/.ssh/authorized_keys`), ""},
		{"moving authorized keys away is a mutation", cmd(`mv ~/.ssh/authorized_keys /tmp/keys`), "persistence.ssh_authorized_keys_command"},
		{"Windows drive root traversal reaches administrator authorized keys", write(`C:\..\ProgramData\ssh\administrators_authorized_keys`), "persistence.ssh_authorized_keys"},
