...
func TestReleasePrecisionPrivilegePolicy(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"clustered no-includes validation is not a modification", cmd("visudo -Icf/etc/sudoers"), ""},
		...
		{"visudo no-includes option targets default policy", cmd("visudo -I"), "privilege.sudoers_tamper"},
		{"visudo clustered no-includes and file option", cmd("visudo -If/etc/sudoers.d/agent"), "privilege.sudoers_tamper"},
		{"visudo positional active policy", cmd("visudo /etc/sudoers.d/agent"), "privilege.sudoers_tamper"},
		...
		{"cp suffix beginning with t is not target-directory", cmd("cp -St/etc/sudoers.d /tmp/policy /tmp/dest"), ""},
...
func TestReleasePrecisionPersistencePolicy(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"authorized keys path used as mv backup suffix stays quiet", cmd(`mv --suffix ~/.ssh/authorized_keys /tmp/source /tmp/dest`), ""},
		{"cp suffix beginning with t is not an SSH target directory", cmd(`cp -St~/.ssh /tmp/authorized_keys /tmp/dest`), ""},
